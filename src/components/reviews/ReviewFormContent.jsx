...
export default function ReviewFormContent({ 
  ...
  // Sincroniza el formulario si cambian las props de forma externa
  useEffect(() => {
    setRanking(userReview?.ranking || 5);
    setComment(userReview?.comment || "");
  }, [userReview, productId]);

  // 1. Estructura el objeto base para la base de datos
  ...
  // Helper personalizado para las alertas de éxito
  ...
  // 2. Función dedicada exclusivamente a CREAR la review
  const handleCreateReview = async () => {
    const reviewData = buildReviewData();
    await addReview(reviewData);
    showSuccessAlert("Review publicada con éxito");
  };

  // 3. Función dedicada exclusivamente a EDITAR la review
  const handleEditReview = async () => {
    const reviewData = buildReviewData();
    await editReview(userReview.id, reviewData);
    showSuccessAlert("Review actualizada con éxito");
  };

  // 4. Manejador del submit que coordina el flujo
